...
public class UnitAssemblerWithMenu extends PayloadBlock{
    ...
    public UnitAssemblerWithMenu(String name){
        super(name);
        update = solid = true;
        rotate = true;
        rotateDraw = false;
    ...
    public boolean canPlaceOn(Tile tile, Team team, int rotation){
        ...
        return
                !indexer.getFlagged(team, BlockFlag.unitAssembler).contains(b -> getRect(Tmp.r2, b.x, b.y, b.rotation).overlaps(rect)) &&
                        !team.data().getBuildings(ConstructBlock.get(size)).contains(b -> ((ConstructBuild)b).current instanceof UnitAssemblerWithMenu && getRect(Tmp.r2, b.x, b.y, b.rotation).overlaps(rect));
    }
    ...
    public void setBars(){
        ...
        addBar("progress", (UnitAssemblerWithMenuBuild e) -> new Bar("bar.progress", Pal.ammo, () -> e.progress));

        addBar("units", (UnitAssemblerWithMenuBuild e) ->
                new Bar(() ->
    ...
    public void init(){
        ...
        consume(consItem = new ConsumeItemDynamic((UnitAssemblerWithMenuBuild build) -> build.plan().itemReq != null ? build.plan().itemReq : ItemStack.empty));
        consume(new ConsumeLiquidsDynamic((UnitAssemblerWithMenuBuild build) -> build.plan().liquidReq != null ? build.plan().liquidReq : LiquidStack.empty));
    ...
    public void initCapacities(){
        ...
        for(CustomAssemblerUnitPlan plan : plans){
    ...
    public static class CustomAssemblerUnitPlan{
        ...
        public CustomAssemblerUnitPlan(UnitType unit, float time, Seq<PayloadStack> requirements){
            this.unit = unit;
            this.time = time;
            this.requirements = requirements;
        }

        CustomAssemblerUnitPlan(){}
    }
    ...
    public static void assemblerUnitSpawned(Tile tile){
        ...
        UnitAssemblerBuild build = (UnitAssemblerBuild) tile.build;
        build.spawned();
    }
    ...
    public class UnitAssemblerWithMenuBuild extends PayloadBlockBuild<Payload>{
        ...
        public int selectedMode = 0; // Индекс выбранного плана
        ...
        public void buildConfiguration(Table table) {
            ...
            UnitAssemblerWithMenu blockType = (UnitAssemblerWithMenu) block;
            for (int i = 0; i < blockType.plans.size; i++) {
                ...
                CustomAssemblerUnitPlan plan = blockType.plans.get(i);

                // Создаем кнопку с иконкой итогового юнита, который будет построен
                Table buttonContent = new Table();
                buttonContent.defaults().size(24f).pad(2f);

                // Добавляем иконку только итогового юнита из плана
                if (plan.unit != null && plan.unit.uiIcon != null) {
                    Image icon = new Image(plan.unit.uiIcon);
                    icon.setScaling(Scaling.fit);
                    buttonContent.add(icon).size(32f);
                }

                // Создаем кнопку с содержимым
                Button button = new Button(Styles.togglet);
                button.add(buttonContent).grow();
    ...
    public Vec2 getUnitSpawn(){
        ...
        public void checkTier(){
            ...
            for(int i = 0; i < modules.size; i++){
                UnitAssemblerModuleBuild mod = modules.get(i);
        ...
        public CustomAssemblerUnitPlan plan(){
            //clamp plan pos
        ...
        public void drawSelect(){
            for(UnitAssemblerModuleBuild module : modules){
        ...
        public void updateTile(){
            ...
            CustomAssemblerUnitPlan plan = plan();
        ...
        public boolean acceptPayload(Building source, Payload payload){
            CustomAssemblerUnitPlan plan = plan();
            boolean b1 = (this.payload == null || (source instanceof UnitAssemblerModuleBuild)) &&
                    plan.requirements.contains(b -> b.item == payload.content() &&
                            blocks.get(payload.content()) < Mathf.round(b.amount * state.rules.unitCost(team)) -
                                    (source instanceof UnitAssemblerModuleBuild && (this.payload != null && this.payload.equals(payload)) ? 1 : 0));
            return b1;
        }
